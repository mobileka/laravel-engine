...
class G_Task
{
    ...
    public $custom_settings = array(':myisam');
    /**
     * Time Savers
     *
     */
    ...
    public function bm($args)  { return $this->basic_migration($args); }
...
EOT;
    ...
    public function migration($args)
    {
        ...
        $class_name = $this->parse_class_name(array_shift($args));
        ...
        $path = $this->bundle_path ? $this->bundle_path : $this->path('migrations');

        // Let's create the path to where the migration will be stored.
        $file_path = $path . date('Y_m_d_His') . strtolower($this->filename);
    ...
    public function basic_migration($args)
    {
        if ( empty($args) ) {
            echo "Error: Please provide a name for your migration.\n";
            return;
        ...
        $this->generate_basic_migration($class_name, $table_name);
    ...
    protected function generate_basic_migration($class_name, $table_name)
    {

        Template::basic_migration_class($class_name);

        $function = $this->add_after(
            '{',
            Template::construct($table_name),
            Template::func('__construct')
        );

        Content::add_after('{', $function);
    ...
    protected function migration_up($table_event, $table_action, $table_name, $args)
    {
        ...
        if ( $table_event === 'create' ) {
            ...
            foreach ($args as $arg) {
                ...
                if (strstr($arg[0], '_id'))
                {
                    ...
                    $relations[] = '$table->foreign(\''.$arg[0].'\')->references(\'id\')->on(\''.$table.'s\')->on_delete(\'cascade\')->on_update(\'cascade\');';
                }*/
            }

            $fields = $this->set_column('increments', 'id') . '->unsigned();';
        ...
        $up = $this->add_after('function($table) {', $fields, $up);

        if ( $table_event !== 'create' ) {
            return $up;
        } else {
            foreach ($args as $arg) {
                if ($this->is_myisam($arg)) {
                    return $up;
                }
            }
            return $this->add_after('function($table) {', '$table->engine = \'InnoDB\';', $up);
        }
    ...
    {
        if (($pos = strpos($class_name, 'create_')) !== false)
        {
            $table = substr($class_name, $pos + 7);
            $table = str_ireplace('_table', '', $table);
            return $table;
        }

        if (($pos = strpos($class_name, '_to_')) !== false)
        {
            $table = substr($class_name, $pos + 4);
            $table = str_ireplace('_table', '', $table);
            return $table;
        }

        if (($pos = strpos($class_name, '_from_')) !== false)
        {
            $table = substr($class_name, $pos + 6);
            $table = str_ireplace('_table', '', $table);
            return $table;
        }
    ...
    public function parse_class_name($class_name)
    {
        ...
        {
            $this->bundle = substr($class_name, 0, $pos);
            $temp = explode('.', $this->bundle);
            ...
            $this->bundle = array_pop($temp);
            $this->bundle_class_prefix = ucfirst($this->bundle);

            $this->bundle_path = path('bundle');

            foreach ($temp as $dir)
            {
                $this->bundle_path .= $dir . DS;           
            }
            $this->bundle_path .= $this->bundle . DS .'migrations' . DS;

            $class_name = ucfirst($this->bundle) . '_' .  str_replace($this->bundle . '::', '', $class_name);
            $this->filename = substr_replace("_$class_name.php", '', 0, strlen($this->bundle) + 1);
        }
        else
        {
            $this->filename = "_$class_name.php";
        }
    ...
    {
        if ($type === 'unsigned') return "\$table->integer('$field')->unsigned()";
        if ($type === 'decimal') return "\$table->decimal('$field', 10, 2)";
        if ($type === 'index') return "\$table->integer('$field')->index()";
        if ($type === 'unsigned_index') return "\$table->integer('$field')->unsigned()->index()";
        return empty($field)
            ? "\$table->$type()"
            : "\$table->$type('$field')";
    }

    protected function is_custom_setting($field)
    {
        return (in_array($field, $this->custom_settings));
    }

    protected function is_myisam($field)
    {   
        $pos = stripos($field, ':myisam');
        return  ($pos !== false)
            ? true
            : false;
    }

    protected function is_unsigned($field)
    {   
        $pos = stripos($field, ':unsigned');
        return  ($pos !== false)
            ? true
            : false;
    }
    ...
    protected function add_columns($args)
    {
        ...
        foreach( $args as $arg ) {
            ...
            //Common::debug(array($field, $type, $setting));

            if ($this->is_custom_setting($field)) {
                continue;
            }
...
class Content {
    ...
    {
        G_Task::$content = str_replace($where, $where . $to_add, G_Task::$content);
...
EOT;
    ...
    public static function new_class($name, $extends_class = null)
    {
        ...
        G_Task::$content = $content;
    }

    public function basic_migration_class($name)
    {
        $content = "<?php require_once('templates/basicTableMigration.php');\n\nclass $name extends basicTableMigration {}";
    ...
    public static function construct($table_name)
    {
